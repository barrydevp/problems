...
fn main() {
    ...
    let mut dp = vec![vec![BigInt::zero(); (n << 1) + 1]; s + 1];
    dp[1][0] = BigInt::from(1);
    dp[1][1] = BigInt::from(1);
    ...
    for i in 2..=s {
        ...
        for j in 2..=(n << 1).min(i) {
            if j % 2 == 0 {
                ...
                dp[i][1] = dp[i][1] + dp[i - 1][j];
            };
            dp[i][j] = dp[i - 1][j - 1];
            if j == (n << 1) {
                r += dp[i][j];
                e += BigInt::from(i as u128) * dp[i][j];
            }
        }
    }
